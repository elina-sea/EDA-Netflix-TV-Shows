...
```
  ...
  Now doubt, there are lots of graphs to compare already. And for example we can say that:
    - Hypergiants have the greates radius among other types. The smallest smallest radius values are recorded among Red Dwarfs;
    - Temperature range of Brown Dwarfs seems to be in the lowest values, so we can say those stars are the coldest ones among others;
    - Supergiants and Hypergiants are the brightest stars;
    - The lowest absolute magnitute of population can be met among Hypergiants and the highest among Brown Dwarfs.

  But even with this amount of graphs it's quite hard to distinguish one type from another. So let's group them all differently to compile a 'profile' of each class.
  
```{r}
plot_grid(p_temp_stype0, p_lum_stype0, p_rad_stype0, p_magn_stype0)
plot_grid(p_temp_stype1, p_lum_stype1, p_rad_stype1, p_magn_stype1)
plot_grid(p_temp_stype2, p_lum_stype2, p_rad_stype2, p_magn_stype2)
plot_grid(p_temp_stype3, p_lum_stype3, p_rad_stype3, p_magn_stype3)
plot_grid(p_temp_stype4, p_lum_stype4, p_rad_stype4, p_magn_stype4)
plot_grid(p_temp_stype5, p_lum_stype5, p_rad_stype5, p_magn_stype5)
```


  Now we can make some exploration on how this values are interconnected and whar are the depenedniec within each type and in general. But we will look into it a bit later.
  
## Summary on distribution graphs

  Among more than 50 graphs above we can observe all kinds of distributions: distinct positive skewed ones (like stype2_luminosity and stype3_luminosity); negative skewed (like sclassM_magnitute_absolute); and almost bell-shaped or normal distribution on graph stype5_radius. Also there's a few of miltimodal distributions, such as sclassM_magnitute_absolute.
  
  
  
# Part 2
  
  In this part we will concentrate on calculating mean of each parameter overall and regardless class/type of stars.
...
mean(stars_temperature)
```
#### Mean of luminosity
```{r  message=FALSE, warning=FALSE, echo=FALSE}
library(ggplot2)

mean(stars_luminosity)
```
#### Mean of radius
```{r  message=FALSE, warning=FALSE, echo=FALSE}
library(ggplot2)
...
mean(stars_radius)
```
#### Mean of absolute magnitute
```{r  message=FALSE, warning=FALSE, echo=FALSE}
library(ggplot2)

mean(stars_magnitute_absolute)
```
...
Mode(stars_temperature)
```
#### Mode of luminosity
```{r message=FALSE, warning=FALSE, echo=FALSE}
library(DescTools)

Mode(stars_luminosity)
```
#### Mode of radius
```{r message=FALSE, warning=FALSE, echo=FALSE}
library(DescTools)
...
Mode(stars_radius)
```
#### Mode of absolute magnitute
```{r message=FALSE, warning=FALSE, echo=FALSE}
library(DescTools)

Mode(stars_magnitute_absolute)
```


  Let's discuss mode a bit. As we can see, the result went a bit off for radius mode. But actually it's not off but rather demonstrates some of the "disadvantages" of mode:
    - mode is unstable when the data consist of a small number of values;
    - data can have more than one mode or no mode at all;
  Though it is often considered to be a disadvantage I'd say that it's just another way to make some conclusion upon given data. If we have several values which are met with the same frequency we can conclude that distribution graph is meant to have several peaks. In other words by computing mode of radius we can already say that the distribution graph of this value is meant to be multimodal. And if we go back to our radius graph we will see that it's indeed multimodal.
  
  
```{r}
ggplot(df_stars, aes(stars_radius)) + geom_histogram(aes(y = ..density..)) + geom_density()
```
...
median(stars_temperature)
```
#### Median of luminosity
```{r message=FALSE, warning=FALSE, echo=FALSE}

median(stars_luminosity)
```
#### Median of radius
```{r message=FALSE, warning=FALSE, echo=FALSE}
...
median(stars_radius)
```  
#### Median of absolute magnitute
```{r message=FALSE, warning=FALSE, echo=FALSE}

median(stars_magnitute_absolute)
```  
  
### Variabilty
#### Temperature
```{r}
Range(stars_temperature)
IQR(stars_temperature)
var(stars_temperature)
```
...
#### Luminosity
```{r}
Range(stars_luminosity)
IQR(stars_luminosity)
var(stars_luminosity)
```
  ...
  From the computations above we can say:
    ...
    So let's `trim` luminosity and try again. By the way, we could have trimmed luminosity, temperature and radius sets in the very beginning as good practice, because those distributions have extrimely skewed graphs.
    
```{r}
library(DescTools)
trim <- Trim(stars_luminosity, trim = 0.1)
var(trim)
```
    We removed 10% of lowest and greatest values and result is still a very large value what proves that deviations from mean are quite big for this parameter. 
...
#### Absolute magnitute
```{r}
Range(stars_magnitute_absolute)
IQR(stars_magnitute_absolute)
var(stars_magnitute_absolute)
```    
  
  
```{r}

#TODO make conclusions from part 2 (mean, mode, etc)
```
